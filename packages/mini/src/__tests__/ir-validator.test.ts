...
describe('Ω IR validator', () => {
  ...
  it('fails closed on malformed required fields', () => {
    const invalid = {
      ...validIR,
      intent: ' ',
      workerPlan: [{ ...validIR.workerPlan[0], workerId: '' }],
      observationSpec: { ...validIR.observationSpec, targets: [''] },
    } as OmegaIR;
    ...
    expect(result.issues.map((issue) => issue.path)).toEqual([
      'intent',
      'workerPlan[0].workerId',
      'observationSpec.targets[0]',
    ]);
  });
  ...
  it('rejects unsupported versions and worker modes', () => {
    const invalid = {
      ...validIR,
      version: 'omega-ir.v2',
      workerPlan: [{ ...validIR.workerPlan[0], mode: 'arbitrary-execution' }],
    } as unknown as OmegaIR;
