...
import { admitOmegaIR } from '../admission-bridge';
...
describe('Ω admission bridge', () => {
  ...
  it('fails closed for a worker mode mismatch', () => {
    const result = admitOmegaIR({
      ...admittedInput,
      ir: { ...ir, workerPlan: [{ ...ir.workerPlan[0], mode: 'local-mutating' }] },
    });
    expect(result.registryMatched).toBe(false);
    expect(result.change.decision).toBe('DENY');
  });
