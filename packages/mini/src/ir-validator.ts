...
export function validateOmegaIR(ir: OmegaIR): OmegaIRValidation {
  ...
  if (ir.version !== 'omega-ir.v1') {
    issues.push({ path: 'version', message: 'unsupported Omega IR version' });
  }
  if (!nonEmpty(ir.intent)) {
    issues.push({ path: 'intent', message: 'intent must be non-empty' });
  }
  ...
  ir.evidenceRefs.forEach((ref, index) => {
    ...
    if (ref.digest !== undefined && !nonEmpty(ref.digest)) {
      issues.push({ path: `evidenceRefs[${index}].digest`, message: 'digest must be non-empty when supplied' });
    }
  });
  ...
  ir.workerPlan.forEach((worker, index) => {
    ...
    if (!['read-only', 'build-test', 'local-mutating', 'external-consequence'].includes(worker.mode)) {
      issues.push({ path: `workerPlan[${index}].mode`, message: 'unsupported worker mode' });
    }
    if (typeof worker.approvalRequired !== 'boolean') {
      issues.push({ path: `workerPlan[${index}].approvalRequired`, message: 'approvalRequired must be boolean' });
    }
  });
  ...
  if (ir.transitionSpec.requestedStateAfter !== undefined && !nonEmpty(ir.transitionSpec.requestedStateAfter)) {
    issues.push({ path: 'transitionSpec.requestedStateAfter', message: 'requestedStateAfter must be non-empty when supplied' });
  }
  if (ir.transitionSpec.consequence !== undefined && !nonEmpty(ir.transitionSpec.consequence)) {
    issues.push({ path: 'transitionSpec.consequence', message: 'consequence must be non-empty when supplied' });
  }
  if (typeof ir.transitionSpec.dryRun !== 'boolean') {
    issues.push({ path: 'transitionSpec.dryRun', message: 'dryRun must be boolean' });
  }

  if (!nonEmpty(ir.observationSpec.observerId)) {
    issues.push({ path: 'observationSpec.observerId', message: 'observerId must be non-empty' });
  }
