import { createHmac, createPrivateKey, createPublicKey, timingSafeEqual, sign, verify, } from 'node:crypto';
 ...
 */
function resolvePublicKey(privateKeyPem, suppliedPublicKey) {
    let derived;
    try {
        ...
        throw new InvalidSigningKeyError(`Ed25519 signing key could not be parsed: ${error instanceof Error ? error.message : String(error)}. Expected a PEM-encoded private key.`);
    }
    if (suppliedPublicKey && normalizePem(suppliedPublicKey) !== normalizePem(derived)) {
        throw new InvalidSigningKeyError('Supplied Ed25519 public key does not match the private key. ' +
 ...
 */
function createSignaturePayload(attestation) {
    return {
 ...
 */
export function verifyEd25519(attestation, publicKey) {
...
export class AttestationService {
    ...
    constructor(config, keyVersion) {
        ...
        if (!key) {
            ...
            throw new MissingSigningKeyError(`No ${algorithm} key available. Pass one to the AttestationService constructor ` +
    ...
    attest(verificationResult, options) {
        ...
        attestation.signature = this.generateSignature(this.createSignaturePayload(attestation), algorithm);
        return attestation;
    }
    /**
    ...
    verify(attestation) {
        ...
        const claimed = attestation.signingAlgorithm || 'HMAC-SHA256';
        ...
        const expectedSignature = this.generateSignature(this.createSignaturePayload(attestation), this.algorithm);
    ...
    generateSignature(payload, algorithm) {
    ...
    createSignaturePayload(attestation) {
    ...
    keyFingerprint() {
        ...
        const material = this.algorithm === 'Ed25519' && this.publicKey ? this.publicKey : this.signingKey;
