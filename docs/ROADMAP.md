...
- **Durable-log recovery provenance — local diagnostics parity implemented, repair policy not complete.** Health, state, observability, SDK, CLI, dashboard, and operator docs now expose the durable-log source, skipped count, reason, and authenticated key source from the same runtime read. This makes partial recovery diagnosable without turning local provenance into a claim of repair, custody, distributed completeness, or operator acknowledgement.
