...
- **Health/readiness observability — local recovery boundary implemented, deployment health not claimed.** `GET /health` remains unauthenticated for probes and returns explicit liveness, readiness, memory-integrity, persistence-codec, and non-secret policy evidence. An enabled corrupt runtime snapshot now degrades readiness with HTTP `503`, while an enabled missing store remains a valid cold start; the web dashboard, typed SDK, and CLI consume the same readiness response and fail closed when degraded. This does not establish deployment orchestration, distributed health coordination, or production availability.
