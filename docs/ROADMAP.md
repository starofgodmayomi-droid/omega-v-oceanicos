...
The next identity-hardening slice requires `x-omega-operator-id` in strict `OMEGA_ADMIN_REQUIRE_ALLOWLIST=on` mode, preventing a JSON body `operatorId` from substituting for the dedicated request identity field across all three administrative mutations. Optional local-development fallback remains compatible. Focused API validation passed 21 tests with formatting and type-check evidence. The header remains a caller assertion checked against an allowlist, not authentication or identity proofing.
