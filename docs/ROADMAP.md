...
- **Durable-log recovery provenance — local diagnostics parity and operator-action routing implemented, repair policy not complete.** Health, state, observability, SDK, CLI, dashboard, and operator docs now expose the durable-log source, skipped count, reason, authenticated key source, and `operatorAction` routing hint from the same runtime read. The action classification distinguishes partial recovery, key rotation review, and their combination; it does not acknowledge, repair, authorize, or deploy a persistence change. Local provenance still does not claim custody, distributed completeness, or recovery.
