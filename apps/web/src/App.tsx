...
export function App(): JSX.Element {
  ...
  const navigate = (item: string) => {
    ...
    if (item === 'Current') { setMode('observe'); setError(''); return; }
    if (stages.includes(stage)) { setMode(stage); setError(''); return; }
    setError(`${item} is not connected to the current runtime yet`);
  };
  return <div className="os-shell">
    <aside className="sidebar"><div className="brand"><span className="brand-mark">Ω∞v</span><span>ECOSYSTEMOS</span></div><div className="side-status"><span className="status-dot" /> Runtime active</div><nav aria-label="Primary navigation">{navGroups.map((group) => <div className="nav-group" key={group.label}><span className="nav-label">{group.label}</span>{group.items.map((item) => <button className={activeNav === item ? 'nav-item active' : 'nav-item'} key={item} onClick={() => navigate(item)}><span className="nav-glyph">{item === 'Current' ? '◈' : '·'}</span>{item}</button>)}</div>)}</nav><div className="sidebar-foot"><span>LOCAL ENVIRONMENT</span><strong>v0.1.0</strong></div></aside>
