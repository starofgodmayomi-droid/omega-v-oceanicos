...
process.env.OMEGA_ED25519_PRIVATE_KEY = privateKey
  ...
  it('signs, verifies, and preserves Ed25519 provenance through the full API loop', async () => {
    const response = await fetch(`${baseUrl}/complete-loop`, {
      ...
      headers: {
        ...
        'x-request-id': 'ed25519-api-1',
      },
      body: JSON.stringify({
        claim: 'API Ed25519 integration is executable',
        category: 'health-check',
        source: {
          system: 'api-ed25519-test',
          version: '0.1.0',
          environment: 'test',
        },
        observedBy: 'jest',
    ...
    expect(JSON.stringify(result.data.attestation)).not.toContain(
    ...
    const verification = (await verificationResponse.json()) as ApiResponse<{
      valid: boolean;
    }>;
